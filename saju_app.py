...
st.set_page_config(page_title="AI 로또 & 사주 번호 추출기", page_icon="🎰", layout="centered")
...
def create_ticket_image(round_no, games_list, time_str, ticket_title="추천 조합"):
    ...
    draw.text((width // 2, y_curr + 30), "1등 당첨을 진심으로 기원합니다!", fill=(140, 140, 140), font=font_main, anchor="mm")
...
with tab_stats:
    ...
    if st.button("🎲 통계 기반 추천 번호 뽑기", use_container_width=True, type="primary", key="btn_stat"):
        if len(available_pool) < 6:
            ...
            st.session_state.generated_source_title = f"통계_{strategy.split()[1]}"
            st.session_state.fortune_story = ""
...
> 위 번호 풀에 높은 가중치를 배정하여, 오늘 회원님에게 가장 길한 에너지의 번호 조합을 완성했습니다.
            """
            st.session_state.generated_source_title = f"사주_{info['short']}"
...
> 수호성이 지닌 고유 주파수의 숫자를 기점으로 6개 번호를 조화롭게 구성했습니다.
            """
            st.session_state.generated_source_title = f"별자리_{z_sign.split()[0]}"
...
if st.session_state.last_generated_games:
    ...
    with st.expander("👁️ 저장될 번호 사진 미리보기"):
        st.image(img_data, caption=f"제 {target_next_round}회 추천 번호표 ({set_title})", use_container_width=True)
